...
import { getCliente, updateCliente } from "@/lib/api/clientes"
...
export default function EditarClientePage() {
  ...
  useEffect(() => {
    ...
    const result = getCliente(params.id as string)
    if (result.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = updateCliente(params.id as string, formData)

    if (result.data) {
      showToast("Cliente atualizado com sucesso", "success")
      router.push(`/clientes/${params.id}`)
    } else {
      showToast(result.erro || "Erro ao atualizar cliente", "error")
    }
